...
import replicas.winyul.dtrs.ClientObject;
import replicas.winyul.dtrs.Event;
import replicas.winyul.dtrs.Logger;

@WebService(endpointInterface="replicas.winyul.dtrs.com.web.service.DTRSWebServices")
...
public class DTRSWebServicesImpl implements DTRSWebServices{
	...
	public String reserveTicket(String participantID, String eventID, String eventType){
		...
		String response;
		System.out.println("-<><><><>==<<><<><><<>");
		System.out.println(participantID + "," + eventID + "," + eventType);
		...
		System.out.println("check if event exist in the correct server");
		if(events.get(eventType).containsKey(eventID)) {
			System.out.println("//checks if event exist in the server");
			if(Event.EventServer(eventID).equalsIgnoreCase(serverName)){
				Event bookedEvent=events.get(eventType).get(eventID);
				System.out.println("Check if this is booked");
					...
					if(clientEvents.containsKey(participantID)) {
						if(clientEvents.get(participantID).containsKey(eventType)) {
							if(!clientEvents.get(participantID).get(eventType).contains(eventID)) {
								...
								System.out.println("Successfully booked participant");
							}else {
						...
						System.out.println("Add event type with an event");
		...
		}else if (!eventID.substring(0, 3).equals(serverID.substring(0,3))) {
			System.out.println("Checking exceeding limit");
				if(!exceedWeeklyLimit(participantID,eventID.substring(4))) {
					...
					System.out.println("End of UDP Message");
					if(serverResponse.startsWith("Success: ")) {
						System.out.println("BOOKED SUCCESSFULLY");
						System.out.println(clientEvents.get(participantID));
						System.out.println(clientEvents.get(participantID).containsKey(eventType));
						if(clientEvents.get(participantID).containsKey(eventType)) {
							System.out.println("ADDING TO CLIENT EVENTS AFTER BOOK");
							clientEvents.get(participantID).get(eventType).add(eventID);
//							try {
...
//							return response;
	...
	//udp methods
	public String removeEventUDP(String participantID,String eventID,String eventType) {
//	public String removeEventUDP(String eventID,String eventType,String participantID) {
	...
	private String sendUDPMessage(int serverPort,String method,String participantID,String eventType,String eventID) {
        ...
        try {
            ...
            result = parts[0];
			System.out.println("RECEIVING DATA " + result);
...
//                    int diff = Math.abs(day2 - day1);
	...
	public String exchangeTickets(String participantID, String eventID, String new_eventID, String new_eventType) {
		...
		for (String eventType :events.keySet()) {
			if (clientEvents.get(participantID).get(eventType) == null)
				continue;

			System.out.println(participantID + " " + eventType + " " + eventID);
			System.out.println(clientEvents.get(participantID));
			System.out.println(clientEvents.get(participantID).get(eventType));
//			System.out.println(clientEvents.get(participantID).get(eventType).contains(eventID));
//			if (clientEvents.get(participantID).get(eventType) == null) {
//				clientEvents.get(participantID).get(eventType).add(eventID);
//			}
   		 	if (clientEvents.get(participantID).get(eventType).contains(eventID)) {
